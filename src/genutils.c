...
**                                       *
******************************************/
void Square(BYTE *A, BYTE *B, int L) { Mult(A, B, L, B, L); }
...
int DivMod(BYTE *x, int lenx, BYTE *n, int lenn, BYTE *quot, BYTE *rem) {
    ...
    do {
      ...
      if (shift) {
        if (ts[0] != 0)
          ...
          for (i = 0; i < lgth_x; i++) ts[i] = ts[i + 1];
          ts[lgth_x] = 0x00;
        }
      }
  ...
  if (lgth_x) memcpy(rem + lenn - lgth_x, tx, lgth_x);
