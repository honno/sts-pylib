...
void quadRes1() {
  ...
  ahtopb(
      "987b6a6bf2c56a97291c445409920032499f9ee7ad128301b5d0254aa1a9633fdbd378d4"
      "0149f1e23a13849f3d45992f5c4c6b7104099bc301f6005f9d8115e1",
      p, 64);
  ahtopb(
      "3844506a9456c564b8b8538e0cc15aff46c95e69600f084f0657c2401b3c244734b62ea9"
      "bb95be4923b9b7e84eeaf1a224894ef0328d44bc3eb3e983644da3f5",
      g, 64);
  num_0s = 0;
  num_1s = 0;
  done = 0;
...
void quadRes2() {
  ...
  ahtopb(
      "7844506a9456c564b8b8538e0cc15aff46c95e69600f084f0657c2401b3c244734b62ea9"
      "bb95be4923b9b7e84eeaf1a224894ef0328d44bc3eb3e983644da3f5",
      g, 64);
...
void cubicRes() {
  ...
  for (k = 0; k < tp.numOfBitStreams; k++) {
    ...
    do {
      ...
      Mult(tmp, g, 64, g, 64);
      Mult(x, tmp, 128, g,
           64);  // Don't need to mod by 2^512, just take low 64 bytes
      memcpy(g, x + 128, 64);
      done = convertToBits(g, 512, tp.n, &num_0s, &num_1s, &bitsRead);
    } while (!done);
    fprintf(freqfp, "\t\tBITSREAD = %d 0s = %d 1s = %d\n", bitsRead, num_0s,
...
void exclusiveOR() {
  ...
  memcpy(bit_sequence,
         "000101101101100100010111100100101001101110110100010000001010111111101"
         "0100100001010110110000000000100110000101110011111111100111",
         127);
  num_0s = 0;
  num_1s = 0;
  bitsRead = 0;
...
void bbs() {
  ...
  ahtopb(
      "E65097BAEC92E70478CAF4ED0ED94E1C94B154466BFB9EC9BE37B2B0FF8526C222B76E0E"
      "915017535AE8B9207250257D0A0C87C0DACEF78E17D1EF9DC44FD91F",
      p, 64);
  ahtopb(
      "E029AEFCF8EA2C29D99CB53DD5FA9BC1D0176F5DF8D9110FD16EE21F32E37BA86FF42F00"
      "531AD5B8A43073182CC2E15F5C86E8DA059E346777C9A985F7D8A867",
      q, 64);
  memset(n, 0x00, 128);
  Mult(n, p, 64, q, 64);
  memset(s, 0x00, 64);
  ahtopb(
      "10d6333cfac8e30e808d2192f7c0439480da79db9bbca1667d73be9a677ed31311f3b830"
      "937763837cb7b1b1dc75f14eea417f84d9625628750de99e7ef1e976",
      s, 64);
  memset(x, 0x00, 256);
  ModSqr(x, s, 64, n, 128);
...
void micali_schnorr() {
  ...
  for (i = 0; i < tp.numOfBitStreams; i++) {
    ...
    do {
      ...
      for (j = 0; j < 3; j++) bshl(Tail, 105);
      done = convertToBits(Tail, k, tp.n, &num_0s, &num_1s, &bitsRead);
      memset(X, 0x00, 128);
      memcpy(X + 104, Y, 24);
      for (j = 0; j < 5; j++) bshr(X + 104, 24);
    } while (!done);
...
//  This is the generic form of the generator found on the last page of the
//  Change Notice for FIPS 186-2
void SHA1() {
